...
router.post('/', async(req,res) =>{
    try{
        const {nome, telefone,email, senha_hash, plano, status} =  req.body;

        if(!nome || !telefone || !email || !senha_hash || !plano || !status){
        ...
        const senhaHash = await bcrypt.hash(senha_hash, 16);
